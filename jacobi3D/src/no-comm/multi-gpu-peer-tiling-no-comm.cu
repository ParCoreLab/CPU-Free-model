...
namespace MultiGPUPeerTilingNoComm {
...
int MultiGPUPeerTilingNoComm::init(int argc, char *argv[]) {
...
#pragma omp barrier
        ...
        CUDA_RT_CALL(cudaLaunchCooperativeKernel((void *)MultiGPUPeerTilingNoComm::jacobi_kernel,
                                                 comp_dim_grid, comp_dim_block, kernelArgsInner, 0,
                                                 inner_domain_stream));

        // CUDA_RT_CALL(cudaLaunchCooperativeKernel(
        //     (void *)MultiGPUPeerTilingNoComm::boundary_sync_kernel, comm_dim_grid,
