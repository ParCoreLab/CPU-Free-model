...
__global__ void initialize_boundaries(real *__restrict__ const a_new, real *__restrict__ const a,
    ...
    for (unsigned int iz = (blockIdx.y * blockDim.y + threadIdx.y) +(blockIdx.x * blockDim.x + threadIdx.x); iz < my_nz;
         iz += blockDim.x * gridDim.x * blockDim.y * gridDim.y) {
...
__global__ void jacobi_kernel_single_gpu(real *__restrict__ const a_new,
    ...
    if (iz < iz_end && iy < (ny - 1) && ix < (nx - 1)) {
        const real new_val = (1/6) * (a[iz * ny * nx + iy * nx + ix + 1] + a[iz * ny * nx + iy * nx + ix - 1] +
...
double single_gpu(const int nx, const int ny, const int nz, const int iter_max, real *const a_ref_h,
    ...
    if (print)
        printf(
            "Single GPU jacobi relaxation (non-persistent kernel): %d iterations on %d x %d x %d mesh "
            "with "
            "norm "
    ...
    dim3 dim_grid((nx + dim_block_x - 1) / dim_block_x, (ny + dim_block_y - 1) / dim_block_y, (nz + dim_block_z - 1) / dim_block_z);
    ...
    while (iter < iter_max) {
        ...
        CUDA_RT_CALL(cudaMemcpyAsync(a_new + iz_end * ny * nx, a_new + iz_start * ny * nx, nx * ny * sizeof(real),
...
double single_gpu_persistent(const int nx, const int ny, const int nz, const int iter_max, real *const a_ref_h,
                             const int nccheck, const bool print) {
    real *a;
    real *a_new;
    ...
    if (print)
        printf(
            "Single GPU jacobi relaxation (persistent kernel): %d iterations on %d x %d x %d mesh with "
            "norm "
            "check every %d iterations\n",
            iter_max, nx, ny, nz, nccheck);
    ...
    void *kernelArgs[] = {(void *)&a_new,   (void *)&a,  (void *)&iz_start,
                          (void *)&iz_end, (void *)&ny,  (void *)&nx, (void *)&calculate_norm,
                          (void *)&iter_max};
...
void report_results(const int nz,const int ny, const int nx, real *a_ref_h, real *a_h, const int num_devices,
    ...
    if (result_correct) {
        ...
        if (compare_to_single_gpu) {
            printf(
                "Non-persistent kernel - %dx%dx%d: 1 GPU: %8.4f s, %d GPUs: %8.4f s, speedup: %8.2f, "
                "efficiency: %8.2f \n",
