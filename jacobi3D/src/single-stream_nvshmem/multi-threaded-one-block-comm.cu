        ...
        while (iter < iter_max)
        {
            if (blockIdx.x == gridDim.x - 1)
            {
                ...
                nvshmemx_putmem_signal_block(
                    ...
                    ny * nx * sizeof(real), &is_done_computing_flags[next_iter_mod * 2 + 1], 1, NVSHMEM_SIGNAL_ADD, top);
                ...
                nvshmemx_putmem_signal_block(
                    ...
                    ny * nx * sizeof(real), &is_done_computing_flags[next_iter_mod * 2], 1, NVSHMEM_SIGNAL_ADD, bottom);
            }
            else
            {
