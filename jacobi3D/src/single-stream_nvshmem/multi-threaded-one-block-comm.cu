...
namespace SSMultiThreadedOneBlockCommNvshmem
{
    ...
    __global__ void __launch_bounds__(1024, 1)
        jacobi_kernel(real *__restrict__ a_new, real *__restrict__ a, const int iz_start, const int iz_end, const int ny,
                      const int nx, const int iter_max, volatile real *__restrict__ halo_buffer_top,
                      volatile real *__restrict__ halo_buffer_bottom, uint64_t *const is_done_computing_flags, const int top,
                      const int bottom)
    {
        ...
        while (iter < iter_max)
        {
            if (blockIdx.x == gridDim.x - 1)
            {
                ...
                nvshmemx_putmem_signal_nbi_block(
                    ((real *)halo_buffer_bottom) + next_iter_mod * ny * nx, a_new + iz_first,
                    ny * nx * sizeof(real), is_done_computing_flags, 1, NVSHMEM_SIGNAL_ADD, top);
                nvshmemx_putmem_signal_nbi_block(
                    ((real *)halo_buffer_top) + next_iter_mod * ny * nx, a_new + iz_last,
