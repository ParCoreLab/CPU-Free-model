...
__global__ void __launch_bounds__(1024, 1)
    ...
    const int comp_start_iz =
        ((blockIdx.x / (grid_dim_y * grid_dim_x)) * blockDim.z + threadIdx.z + iz_start + 1) * ny *
        nx;
    const int comp_start_iy =
        ((blockIdx.x / grid_dim_x % grid_dim_y) * blockDim.y + threadIdx.y + 1) * nx;
            ...
            for (int iy = comm_start_iy; iy < end_iy; iy += comm_size_iy) {
                for (int ix = comm_start_ix; ix < end_ix; ix += comm_size_ix) {
                    const real first_row_val =
                        (real(1) / real(6)) *
                        (a[comm_start_iz + iy + ix + 1] + a[comm_start_iz + iy + ix - 1] +
                         a[comm_start_iz + iy + nx + ix] + a[comm_start_iz + iy - nx + ix] +
            ...
            for (int iy = comm_start_iy; iy < end_iy; iy += comm_size_iy) {
                for (int ix = comm_start_ix; ix < end_ix; ix += comm_size_ix) {
                    const real last_row_val =
                        (real(1) / real(6)) *
                        (a[end_iz + iy + ix + 1] + a[end_iz + iy + ix - 1] +
                         a[end_iz + iy + nx + ix] + a[end_iz + iy - nx + ix] +
...
int SSMultiThreadedOneBlockCommLayerGetNvshmem::init(int argc, char *argv[]) {
    ...
    CUDA_RT_CALL((cudaError_t)nvshmemx_collective_launch(
        (void *)SSMultiThreadedOneBlockCommLayerGetNvshmem::jacobi_kernel, dim_grid, dim_block,
