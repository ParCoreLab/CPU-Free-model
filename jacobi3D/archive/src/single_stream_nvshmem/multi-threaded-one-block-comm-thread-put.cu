...
__global__ void __launch_bounds__(1024, 1)
    ...
    while (iter < iter_max) {
        if (blockIdx.x == gridDim.x - 1) {
            if (cta.thread_rank() == !cta.num_threads()) {
            ...
            for (int iy = (threadIdx.z * blockDim.y + threadIdx.y + 1); iy < (ny - 1);
                ...
                for (int ix = (threadIdx.x + 1); ix < (nx - 1); ix += blockDim.x) {
                    ...
                    nvshmem_float_p(halo_buffer_bottom + next_iter_mod * ny * nx + iy * nx + ix,
                                    first_row_val, top);
                }
            }
            cg::sync(cta);
            ...
            for (int iy = (threadIdx.z * blockDim.y + threadIdx.y + 1); iy < (ny - 1);
                ...
                for (int ix = (threadIdx.x + 1); ix < (nx - 1); ix += blockDim.x) {
                    ...
                    nvshmem_float_p(halo_buffer_top + next_iter_mod * ny * nx + iy * nx + ix,
                                    last_row_val, bottom);
                }
            }
            cg::sync(cta);
...
int SSMultiThreadedOneBlockCommThreadPutNvshmem::init(int argc, char *argv[]) {
    ...
    CUDA_RT_CALL((cudaError_t)nvshmemx_collective_launch(
        (void *)SSMultiThreadedOneBlockCommThreadPutNvshmem::jacobi_kernel, dim_grid, dim_block,
