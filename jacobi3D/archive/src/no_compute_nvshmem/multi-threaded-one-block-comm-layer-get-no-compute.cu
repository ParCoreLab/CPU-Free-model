...
__global__ void __launch_bounds__(1024, 1)
    ...
    while (iter < iter_max) {
        if (blockIdx.x == gridDim.x - 1) {
            ...
            nvshmemx_getmem_block(halo_buffer_top + cur_iter_mod * ny * nx,
                                  halo_buffer_bottom + cur_iter_mod * ny * nx,
                                  ny * nx * sizeof(real), top);
            /*
            ...
            * nx + ix] + halo_buffer_top[cur_iter_mod * ny * nx + iy * nx + ix]); a_new[iz_start *
            ny * nx + iy * nx + ix] = first_row_val; halo_buffer_top[next_iter_mod * ny * nx + iy *
            nx + ix] = first_row_val;
                }
            }
            cg::sync(cta);
            */
            ...
            nvshmemx_getmem_block(halo_buffer_bottom + cur_iter_mod * ny * nx,
                                  halo_buffer_top + cur_iter_mod * ny * nx, ny * nx * sizeof(real),
                                  bottom);
            /*
            for (int iy = (threadIdx.z * blockDim.y + threadIdx.y + 1); iy < (ny - 1); iy +=
            blockDim.y * blockDim.z)
            ...
            * ny * nx + iy * nx + ix] + a[(iz_end - 2) * ny * nx + iy * nx + ix]); a_new[(iz_end -
            1) * ny * nx + iy * nx + ix] = last_row_val; halo_buffer_bottom[next_iter_mod * ny * nx
            + iy * nx + ix] = last_row_val;
                }
            }
            cg::sync(cta);
            */
                 ...
                 for (int iy = (threadIdx.y + 1) * nx; iy < (ny - 1) * nx; iy += blockDim.y * nx)
                 {
                     for (int ix = (threadIdx.x + 1); ix < (nx - 1); ix += blockDim.x)
                     {
                         a_new[iz + iy + ix] = (real(1) / real(6)) *
                                               (a[iz + iy + ix + 1] + a[iz + iy + ix - 1] + a[iz +
            iy + nx + ix] + a[iz + iy - nx + ix] + a[iz + ny * nx + iy + ix] + a[iz - ny * nx + iy +
            ix]);
                     }
                 }
             }
...
int SSMultiThreadedOneBlockCommLayerGetNvshmemNoCompute::init(int argc, char *argv[]) {
    ...
    if (top_pe != mype) {
        ...
        if (top_pe != bottom_pe) {
            ...
            if (canAccessPeer) {
                ...
                std::cerr << "P2P access required from " << mype << " to " << bottom_pe
                          << std::endl;
            }
        }
    }
    ...
    CUDA_RT_CALL((cudaError_t)nvshmemx_collective_launch(
        (void *)SSMultiThreadedOneBlockCommLayerGetNvshmemNoCompute::jacobi_kernel, dim_grid,
