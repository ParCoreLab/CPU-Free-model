...
__global__ void __launch_bounds__(1024, 1)
    ...
    while (iter < iter_max) {
        if (blockIdx.x == gridDim.x - 1) {
            ...
            nvshmemx_getmem_nbi_block(halo_buffer_bottom + cur_iter_mod * ny * nx,
                                      halo_buffer_top + cur_iter_mod * ny * nx,
...
int SSMultiThreadedOneBlockCommLayerGetOverlapNvshmemNoCompute::init(int argc, char *argv[]) {
    ...
    CUDA_RT_CALL((cudaError_t)nvshmemx_collective_launch(
        (void *)SSMultiThreadedOneBlockCommLayerGetOverlapNvshmemNoCompute::jacobi_kernel, dim_grid,
