...
namespace SSMultiThreadedOneBlockCommThreadPutNvshmemNoCompute {
    ...
    while (iter < iter_max) {
        if (blockIdx.x == gridDim.x - 1) {
            ...
            * nx + ix] + halo_buffer_top[cur_iter_mod * ny * nx + iy * nx + ix]); a_new[iz_start *
            ny * nx + iy * nx + ix] = first_row_val; nvshmem_float_p(halo_buffer_bottom +
            next_iter_mod * ny * nx + iy * nx + ix,first_row_val,top);
                }
            }
            cg::sync(cta);
            */
            ...
            for (int iy = (threadIdx.z * blockDim.y + threadIdx.y + 1); iy < (ny - 1);
                ...
                for (int ix = (threadIdx.x + 1); ix < (nx - 1); ix += blockDim.x) {
                    nvshmem_float_p(halo_buffer_bottom + next_iter_mod * ny * nx + iy * nx + ix, 0,
                                    top);
                }
            }
            cg::sync(cta);
            ...
            * ny * nx + iy * nx + ix] + a[(iz_end - 2) * ny * nx + iy * nx + ix]); a_new[(iz_end -
            1) * ny * nx + iy * nx + ix] = last_row_val; nvshmem_float_p(halo_buffer_top +
            next_iter_mod * ny * nx + iy * nx + ix,last_row_val,bottom);
                }
            }
            cg::sync(cta);
            */
            ...
            for (int iy = (threadIdx.z * blockDim.y + threadIdx.y + 1); iy < (ny - 1);
                 iy += blockDim.y * blockDim.z) {
                for (int ix = (threadIdx.x + 1); ix < (nx - 1); ix += blockDim.x) {
                    nvshmem_float_p(halo_buffer_top + next_iter_mod * ny * nx + iy * nx + ix, 0,
                                    bottom);
                }
            }
            cg::sync(cta);
...
int SSMultiThreadedOneBlockCommThreadPutNvshmemNoCompute::init(int argc, char *argv[]) {
    ...
    CUDA_RT_CALL((cudaError_t)nvshmemx_collective_launch(
        (void *)SSMultiThreadedOneBlockCommThreadPutNvshmemNoCompute::jacobi_kernel, dim_grid,
