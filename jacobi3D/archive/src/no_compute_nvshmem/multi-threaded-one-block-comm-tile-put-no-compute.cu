...
namespace SSMultiThreadedOneBlockCommTilePutNvshmemNoCompute {
...
__global__ void __launch_bounds__(1024, 1)
    ...
    const int num_comm_tiles_y =
        ny / (blockDim.y * blockDim.z) + (ny % (blockDim.y * blockDim.z) != 0);
        ...
        if (blockIdx.x == gridDim.x - 1) {
                ...
                for (int comm_tile_idx_x = 0; comm_tile_idx_x < num_comm_tiles_x;
                     comm_tile_idx_x++, ix += blockDim.x) {
                        ...
                        */
                        ...
                        {
                        nvshmem_fence();
                        nvshmemx_signal_op(is_done_computing_flags + next_iter_mod * num_flags +
                 ...
                 iz < (iz_end - 1) * ny * nx; iz += (gridDim.x - 1) * blockDim.z * ny * nx)
            {
                ...
                for (int iy = (threadIdx.y + 1) * nx; iy < (ny - 1) * nx; iy += blockDim.y * nx)
                {
                    ...
                    {
                        const real new_val = (real(1) / real(6)) * (a[iz + iy + ix + 1] + a[iz + iy
            + ix - 1] + a[iz + iy_below + ix] + a[iz + iy_above + ix] + a[iz_below + iy + ix] +
            a[iz_above + iy + ix]);
...
int SSMultiThreadedOneBlockCommTilePutNvshmemNoCompute::init(int argc, char *argv[]) {
    ...
    long long unsigned int mesh_size_per_rank =
        2 * nx * ny + 2 * num_comm_tiles_x * dim_block_y * num_comm_tiles_y;
    ...
    CUDA_RT_CALL((cudaError_t)nvshmemx_collective_launch(
        (void *)SSMultiThreadedOneBlockCommTilePutNvshmemNoCompute::jacobi_kernel, dim_grid,
