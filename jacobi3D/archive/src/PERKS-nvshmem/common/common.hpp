...
static T *getRandom3DArray(int height, int width_y, int width_x) {
    T(*a)[width_y][width_x] = (T(*)[width_y][width_x]) new T[height * width_y * width_x];
...
static T *getRandom4DArray(int depth, int height, int width_y, int width_x) {
    ...
    [height][width_y][width_x] =
        (T(*)[height][width_y][width_x]) new T[depth * height * width_y * width_x];
...
template <typename T>
static double checkError3D(int width_y, int width_x, const T *l_output, const T *l_reference,
                           int z_lb, int z_ub, int y_lb, int y_ub, int x_lb, int x_ub) {
    const T(*output)[width_y][width_x] = (const T(*)[width_y][width_x])(l_output);
    const T(*reference)[width_y][width_x] = (const T(*)[width_y][width_x])(l_reference);
    double error = 0.0;
    double max_error = 0.0;
...
template <typename T>
static double checkError4D(int height, int width_y, int width_x, const T *l_output,
                           const T *l_reference, int w_lb, int w_ub, int z_lb, int z_ub, int y_lb,
                           int y_ub, int x_lb, int x_ub) {
    const T(*output)[height][width_y][width_x] = (const T(*)[height][width_y][width_x])(l_output);
    const T(*reference)[height][width_y][width_x] =
        (const T(*)[height][width_y][width_x])(l_reference);
    double error = 0.0;
    double max_error = 0.0;
                ...
                for (int k = x_lb; k < x_ub; k++) {
                    ...
                    if (curr_error > max_error) {
                        printf("Values at index (%d,%d,%d,%d) differ : %.6f and %.6f\n", d, i, j, k,
                               reference[d][i][j][k], output[d][i][j][k]);
                        max_error = curr_error;
                        max_d = d;
                        max_k = k;
                        max_j = j;
...
template <>
double checkError3D<double4>(int width_y, int width_x, const double4 *l_output,
                             const double4 *l_reference, int z_lb, int z_ub, int y_lb, int y_ub,
                             int x_lb, int x_ub) {
    const double4(*output)[width_y][width_x] = (const double4(*)[width_y][width_x])(l_output);
    const double4(*reference)[width_y][width_x] = (const double4(*)[width_y][width_x])(l_reference);
    double error = 0.0;
    double max_error = 0.0;
            ...
            for (int k = x_lb; k < x_ub; k++) {
                ...
                if (curr_error > max_error) {
                    printf("Values x at index (%d,%d,%d) differ : %.6f and %.6f\n", i, j, k,
                           reference[i][j][k].x, output[i][j][k].x);
                    max_error = curr_error;
                    max_k = k;
                    max_j = j;
                    max_i = i;
                ...
                if (curr_error > max_error) {
                    printf("Values y at index (%d,%d,%d) differ : %.6f and %.6f\n", i, j, k,
                           reference[i][j][k].y, output[i][j][k].y);
                    max_error = curr_error;
                    max_k = k;
                    max_j = j;
                    max_i = i;
                ...
                if (curr_error > max_error) {
                    printf("Values z at index (%d,%d,%d) differ : %.6f and %.6f\n", i, j, k,
                           reference[i][j][k].z, output[i][j][k].z);
                    max_error = curr_error;
                    max_k = k;
                    max_j = j;
                    max_i = i;
                ...
                if (curr_error > max_error) {
                    printf("Values w at index (%d,%d,%d) differ : %.6f and %.6f\n", i, j, k,
                           reference[i][j][k].w, output[i][j][k].w);
                    max_error = curr_error;
                    max_k = k;
                    max_j = j;
                    max_i = i;
...
template <>
double checkError3D<float4>(int width_y, int width_x, const float4 *l_output,
                            const float4 *l_reference, int z_lb, int z_ub, int y_lb, int y_ub,
                            int x_lb, int x_ub) {
    const float4(*output)[width_y][width_x] = (const float4(*)[width_y][width_x])(l_output);
    const float4(*reference)[width_y][width_x] = (const float4(*)[width_y][width_x])(l_reference);
    double error = 0.0;
    double max_error = 0.0;
    int max_k = 0, max_j = 0, max_i = 0, max_field = 0;
    for (int i = z_lb; i < z_ub; i++)
...
static void print2DArray(int width_x, const T *l_array, int y_lb, int y_ub, int x_lb, int x_ub) {
    ...
    for (int j = y_lb; j < y_ub; j++) {
        printf("\t");
        for (int k = x_lb; k < x_ub; k++) printf(" %e", array[j][k]);
        printf("\n");
    }
}
...
static void print3DArray(int width_y, int width_x, const T *l_array, int z_lb, int z_ub, int y_lb,
    ...
    for (int i = z_lb; i < z_ub; i++) {
        ...
        for (int j = y_lb; j < y_ub; j++) {
            printf("\t");
            for (int k = x_lb; k < x_ub; k++) printf(" %e", array[i][j][k]);
            printf("\n");
        }
    }
}
...
template <>
void print3DArray<float4>(int width_y, int width_x, const float4 *l_array, int z_lb, int z_ub,
                          int y_lb, int y_ub, int x_lb, int x_ub) {
    const float4(*array)[width_y][width_x] = (float4(*)[width_y][width_x])l_array;
    for (int i = z_lb; i < z_ub; i++) {
        printf("\tLevel : %d\n", i);
        for (int j = y_lb; j < y_ub; j++) {
            printf("\t");
            for (int k = x_lb; k < x_ub; k++)
                printf(" [%e, %e, %e, %e]", array[i][j][k].x, array[i][j][k].y, array[i][j][k].z,
                       array[i][j][k].w);
            printf("\n");
        }
    }
