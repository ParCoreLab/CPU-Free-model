...
__global__ void __launch_bounds__(1024, 1)
    ...
    // const int comp_size_iz = ((gridDim.x - 1) / (grid_dim_y * grid_dim_x)) * blockDim.z * ny *
    // nx; const int comp_size_iy = grid_dim_y * blockDim.y * nx; const int comp_size_ix =
    // grid_dim_x * blockDim.x;

    // const int comp_start_iz = ((blockIdx.x / (grid_dim_y * grid_dim_x)) * blockDim.z +
    // threadIdx.z + iz_start + 1) * ny * nx; const int comp_start_iy = ((blockIdx.x / grid_dim_x %
    // grid_dim_y) * blockDim.y + threadIdx.y + 1) * nx; const int comp_start_ix = ((blockIdx.x %
    // grid_dim_x) * blockDim.x + threadIdx.x + 1);
        ...
        if (blockIdx.x == gridDim.x - 1) {
            ...
            for (int comm_tile_idx_y = 0; comm_tile_idx_y < num_comm_tiles_y;
                ...
                for (int comm_tile_idx_x = 0; comm_tile_idx_x < num_comm_tiles_x;
                    ...
                    * ny * nx + iy + ix] + a[end_iz - ny * nx + iy + ix]);
                    ...
                    */
                    ...
                    if (iy < end_iy && ix < end_ix) {
                        const real last_row_val =
                            remote_my_halo_buffer_on_bottom_neighbor[cur_iter_mod * ny * nx + iy +
                                                                     ix];
                ...
                for (int iy = comp_start_iy; iy < end_iy; iy += comp_size_iy)
                {
                    for (int ix = comp_start_ix; ix < end_ix; ix += comp_size_ix)
                    {
                        a_new[iz + iy + ix] = (real(1) / real(6)) *
                                              (a[iz + iy + ix + 1] + a[iz + iy + ix - 1] + a[iz + iy
            + nx + ix] + a[iz + iy - nx + ix] + a[iz + ny * nx + iy + ix] + a[iz - ny * nx + iy +
            ix]);
                    }
                }
            }
...
#pragma omp barrier
        ...
        // int num_comp_tiles_z = nz_per_gpu / comp_tile_size_z + (nz_per_gpu % comp_tile_size_z !=
        // 0);
