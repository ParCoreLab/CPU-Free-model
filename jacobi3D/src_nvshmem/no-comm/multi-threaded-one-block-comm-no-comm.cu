...
__global__ void __launch_bounds__(1024, 1)
    ...
    while (iter < iter_max) {
        if (blockIdx.x == gridDim.x - 1) {
            ...
            //     halo_buffer_top + next_iter_mod * ny * nx, a_new + end_iz, ny * nx *
            //     sizeof(real), is_done_computing_flags + next_iter_mod * 2, iter + 1,
...
int SSMultiThreadedOneBlockCommNvshmemNoComm::init(int argc, char *argv[]) {
    ...
    CUDA_RT_CALL((cudaError_t)nvshmemx_collective_launch(
        (void *)SSMultiThreadedOneBlockCommNvshmemNoComm::jacobi_kernel, dim_grid, dim_block,
