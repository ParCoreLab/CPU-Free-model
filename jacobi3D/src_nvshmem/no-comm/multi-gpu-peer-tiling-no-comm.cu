...
int MultiGPUPeerTilingNvshmemNoComm::init(int argc, char *argv[]) {
    ...
    CUDA_RT_CALL(cudaLaunchCooperativeKernel(
        (void *)MultiGPUPeerTilingNvshmemNoComm::jacobi_kernel, comp_dim_grid, comp_dim_block,
        kernelArgsInner, 0, inner_domain_stream));

    // CUDA_RT_CALL((cudaError_t)nvshmemx_collective_launch(
    //     (void *)MultiGPUPeerTilingNvshmemNoComm::boundary_sync_kernel, comm_dim_grid,
