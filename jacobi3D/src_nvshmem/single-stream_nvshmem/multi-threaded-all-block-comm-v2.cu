...
__global__ void __launch_bounds__(1024, 1)
    ...
    const int base_idx_comm = blockIdx.x * thread_count_per_block +
    ...
    while (iter < iter_max) {
        ...
        // or keep it at 2 flags but count the num of blocks in transport per layer.
        ...
        for (int comm_element_idx = base_idx_comm + nx + 1; comm_element_idx < ny * nx - nx-1;
             comm_element_idx += gridDim.x * thread_count_per_block) {
            int idx_x = comm_element_idx % nx;
            int prev_idx_x = idx_x != 0 ? comm_element_idx - 1 : -1;
            int next_idx_x = idx_x % nx != nx - 1 ? comm_element_idx + 1 : -1;

            if (prev_idx_x > 0 && next_idx_x > 0) {
                real new_val =
                    neighbour_coeff *
                    (a[iz_begin + next_idx_x] + a[iz_begin + prev_idx_x] +
                     a[iz_begin + comm_element_idx + nx] + a[iz_begin + comm_element_idx - nx] +
                     a[iz_begin_next + comm_element_idx] +
                     halo_buffer_of_top_neighbor[cur_iter_mod * ny * nx + comm_element_idx]);

                a_new[iz_begin + comm_element_idx] = new_val;

                new_val =
                    neighbour_coeff *
                    (a[iz_finish + next_idx_x] + a[iz_finish + prev_idx_x] +
                     a[iz_finish + comm_element_idx + nx] + a[iz_finish + comm_element_idx - nx] +
                     halo_buffer_of_bottom_neighbor[cur_iter_mod * ny * nx + comm_element_idx] +
                     a[iz_finish_prev + iy * nx + ix]);

                a_new[iz_finish + iy + ix] = new_val;
            }
        }

        // this is needed since a block must finish its computation or is it ?
        cg::sync(cta);

        for (int copy_block_idx = blockIdx.x; ny * nx - copy_block_idx * thread_count_per_block > 0;
             copy_block_idx += gridDim.x) {
            nvshmemx_float_put_signal_nbi_block(
                halo_buffer_of_bottom_neighbor + (next_iter_mod * ny * nx) +
                    (copy_block_idx * thread_count_per_block),
                a_new + iz_start * ny * nx + copy_block_idx * thread_count_per_block,
                min(thread_count_per_block, ny * nx - copy_block_idx * thread_count_per_block),
                &is_done_computing[0], 1, NVSHMEM_SIGNAL_ADD, top);
            nvshmemx_float_put_signal_nbi_block(
                halo_buffer_of_top_neighbor + next_iter_mod * ny * nx +
                    copy_block_idx * thread_count_per_block,
                a_new + (iz_end - 1) * ny * nx + copy_block_idx * thread_count_per_block,
                min(thread_count_per_block, ny * nx - copy_block_idx * thread_count_per_block),
                &is_done_computing[1], 1, NVSHMEM_SIGNAL_ADD, bottom);
        }

        for (iz = (base_iz_comp + iz_start + 1) * ny * nx; iz < (iz_end - 1) * ny * nx;
             iz += comp_tile_size_z * ny * nx) {
            int iz_below = iz + ny * nx;
            int iz_above = iz - ny * nx;
            for (iy = (base_iy_comp + 1) * nx; iy < (ny - 1) * nx; iy += comp_tile_size_y * nx) {
                int iy_below = iy + nx;
                int iy_above = iy - nx;
                for (ix = (base_ix_comp + 1); ix < (nx - 1); ix += comp_tile_size_x) {
                    const real new_val =
                        neighbour_coeff *
