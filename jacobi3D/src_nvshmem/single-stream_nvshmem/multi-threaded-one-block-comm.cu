    ...
    while (iter < iter_max) {
        if (blockIdx.x == gridDim.x - 1) {
            ...
            nvshmemx_float_put_signal_nbi_block(
                halo_buffer_for_top_neighbor + next_iter_mod * ny * nx, a_new + iz_start * ny * nx,
            ...
            for (iy = (base_iy + 1) * nx; iy < (ny - 1) * nx; iy += comm_tile_size_y * nx) {
                ...
                for (ix = (base_ix + 1); ix < (nx - 1); ix += comm_tile_size_x) {
                    const real new_val =
                        (a[iz + iy + ix + 1] + a[iz + iy + ix - 1] + a[iz + iy_below + ix] +
                         a[iz + iy_above + ix] +
                         halo_buffer_for_bottom_neighbor[cur_iter_mod * ny * nx + iy * nx + ix] +
                         a[iz_above + iy * nx + ix]) /
                        real(6.0);
            ...
            nvshmemx_float_put_signal_nbi_block(
                halo_buffer_for_bottom_neighbor + next_iter_mod * ny * nx,
