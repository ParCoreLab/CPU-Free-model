...
__global__ void __launch_bounds__(1024, 1)
    ...
    while (iter < iter_max) {
        ...
        // for loops needs to be changed to accomidate multiple blocks in comm // does thıs work
        // with changes to base_iy_comm? does it overcompute ??
        for (iy = (base_iy_comm + 1) * nx; iy < (ny - 1) * nx; iy += comm_tile_size_y * nx) {
            int iy_below = iy + nx;
            int iy_above = iy - nx;
            for (ix = (base_ix_comm + 1); ix < (nx - 1); ix += comm_tile_size_x) {
                // big bottleneck here
                const real new_val =
        ...
        //this is needed since the computation pattern is different with communication pattern
        cg::sync(grid);
        ...
        // big problem?? or not much latency on this point???
