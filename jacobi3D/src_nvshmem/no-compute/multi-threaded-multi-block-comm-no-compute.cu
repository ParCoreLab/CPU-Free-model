                      ...
                      const int bottom)
    {
        ...
        while (iter < iter_max)
        {
            if (comm_block_id < comm_sm_count_per_layer)
            {
                ...
                */
                ...
                nvshmemx_putmem_signal_nbi_block(
                    halo_buffer_bottom + next_iter_mod * ny * nx + comm_start_block_y * nx + comm_start_block_x,
            ...
            else if (comm_block_id < 2 * comm_sm_count_per_layer)
            {
                if (!cta.thread_rank())
                {
                    nvshmem_signal_wait_until(is_done_computing_flags + cur_iter_mod * 2 * comm_sm_count_per_layer + comm_sm_count_per_layer + comm_block_id-comm_sm_count_per_layer, NVSHMEM_CMP_EQ, iter);
                }
                /*
                cg::sync(cta);
