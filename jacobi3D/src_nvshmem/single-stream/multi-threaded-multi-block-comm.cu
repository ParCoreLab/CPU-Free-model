...
namespace SSMultiThreadedMultiBlockCommNvshmem
{
                      ...
                      const int bottom)
    {
        ...
        const int iz_dim = blockDim.z * ny * nx;
        const int iy_dim = blockDim.y * nx;
        const int ix_dim = blockDim.x;

        const int comp_start_iz = ((blockIdx.x * comp_block_count_per_sm / (tile_count_x * tile_count_y)) * blockDim.z + threadIdx.z + iz_start + 1) * ny * nx;
        const int comp_start_iy = ((((blockIdx.x * comp_block_count_per_sm) / tile_count_x) % tile_count_y) * blockDim.y + threadIdx.y + 1) * nx;
        ...
        const size_t total_comm_size = min(comm_block_count_per_sm * cta.num_threads(), ny * nx - comm_start_block_y - comm_start_block_x)* sizeof(real);
...
int SSMultiThreadedMultiBlockCommNvshmem::init(int argc, char *argv[])
{
    ...
    CUDA_RT_CALL((cudaError_t)nvshmemx_collective_launch(
        (void *)SSMultiThreadedMultiBlockCommNvshmem::jacobi_kernel, dim_grid, dim_block, kernelArgs,
        0, nullptr));
