...
#include "../../include/single-stream/single-stream-nvshmem-two-block.cuh"

namespace cg = cooperative_groups;

namespace SSTwoBlockCommNVSHMEM {
        ...
        if (blockIdx.x == gridDim.x - 1) {
            ...
            for (int comm_tile_idx = 0; comm_tile_idx < num_comm_tiles; comm_tile_idx++) {
                ...
                if (cta.thread_rank() == 0) {
                    ...
                    // int pe = nvshmem_n_pes();
                    // nvshmemx_int_signal(&remote_am_done_writing_to_top_neighbor[next_iter_comm_tile_flag_idx], iter, (pe+1) % 2);
                    // nvshmem_float_p(a_new + top_iy, 1, 1);
                }
            }
...
int SSTwoBlockCommNVSHMEM::init(int argc, char *argv[]) {
...
#pragma omp barrier
        ...
        CUDA_RT_CALL(cudaLaunchCooperativeKernel((void *)SSTwoBlockCommNVSHMEM::jacobi_kernel,
