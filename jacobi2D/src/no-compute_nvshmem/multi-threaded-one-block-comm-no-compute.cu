...
#include "../../include/no-compute_nvshmem/multi-threaded-one-block-comm-no-compute.cuh"
...
int SSMultiThreadedOneBlockCommNvshmemNoCompute::init(int argc, char *argv[])
{
    ...
    CUDA_RT_CALL((cudaError_t)nvshmemx_collective_launch(
        (void *)SSMultiThreadedOneBlockCommNvshmemNoCompute::jacobi_kernel, dim_grid, dim_block, kernelArgs,
        0, nullptr));
