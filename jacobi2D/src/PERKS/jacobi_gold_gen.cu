...
[[maybe_unused]] static void print_matrix(int width_y, int width_x, real* l_output) {
    //    real(*output)[width_x] = (real(*)[width_x])l_output;
    for (int i = 1; i < width_y - 1; i++)
        for (int j = 1; j < width_x - 1; j++)
            printf("Output[%d][%d] = %.6f\n", i, j, l_output[j * width_y + i]);
}

static void jacobi_step(const real* input, int width_y, int width_x, real* output, int step) {
    for (int l_y = 0; l_y < width_y - 0; l_y++) {
        for (int l_x = 0; l_x < width_x - 0; l_x++) {
            ...
            output[l_y * width_y + l_x] =
                (5 * input[(l_y + s) * width_y + l_x] + 12 * input[l_y * width_y + l_x + w] +
                 15 * input[l_y * width_y + l_x] + 12 * input[l_y * width_y + l_x + e] +
                 5 * input[(l_y + n) * width_y + l_x]) /
                118;
        }
    }
}
