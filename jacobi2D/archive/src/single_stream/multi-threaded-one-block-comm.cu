...
__global__ void __launch_bounds__(1024, 1)
    jacobi_kernel(real *a_new, real *a, const int iy_start, const int iy_end, const int nx,
                  const int num_comm_tiles, const int iter_max,
    ...
    while (iter < iter_max) {
        if (blockIdx.x == gridDim.x - 1) {
            ...
            for (int comm_tile_idx = 0; comm_tile_idx < num_comm_tiles;
                 comm_tile_idx++, ix += blockDim.x * blockDim.y) {
                if (!cta.thread_rank()) {
            ...
            for (int iy = (blockIdx.x * blockDim.y + threadIdx.y + iy_start + 1) * nx;
                ...
                for (int ix = threadIdx.x + 1; ix < (nx - 1); ix += blockDim.x) {
                    const real new_val = 0.25 * (a[iy + ix + 1] + a[iy + ix - 1] + a[iy + nx + ix] +
                                                 a[iy - nx + ix]);
                    a_new[iy + ix] = new_val;
                }
            }
