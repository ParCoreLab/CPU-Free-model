...
__global__ void __launch_bounds__(1024, 1)
    ...
    while (iter < iter_max) {
        if (blockIdx.x == gridDim.x - 1) {
            ...
            //                 a[comm_start_iy + nx + ix] + halo_buffer_top[cur_iter_mod * nx +
            //                 ix]);
            //     a_new[comm_start_iy + ix] = first_row_val;
            // }

            // nvshmemx_putmem_signal_nbi_block(
            //     halo_buffer_bottom + next_iter_mod * nx, a_new + comm_start_iy, nx *
            //     sizeof(real), is_done_computing_flags + next_iter_mod * 2 + 1, iter + 1,
