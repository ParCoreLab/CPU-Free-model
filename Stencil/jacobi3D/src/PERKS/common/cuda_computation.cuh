...
#define stencilParaList                                                                           \
    const REAL west[HALO], const REAL east[HALO], const REAL north[HALO], const REAL south[HALO], \
        const REAL top[HALO], const REAL bottom[HALO], const REAL center
...
#define stencilParaT                                                      \
    const REAL filter[3][3][3] = {{{0.5 / 159, 0.7 / 159, 0.90 / 159},    \
...
#define stencilParaT                                                         \
    const REAL filter[3][3][3] = {                                           \
        {{0, -0.0833f, 0}, {-0.0833f, -0.166f, -0.0833f}, {0, -0.0833f, 0}}, \
...
#define stencilParaT                                                                     \
    const REAL filter[3][3][3] = {                                                       \
        {{0.50 / 159, 0.0, 0.50 / 159}, {0.0, 0.0, 0.0}, {0.50 / 159, 0.0, 0.50 / 159}}, \
        {{0.51 / 159, 0.71 / 159, 0.91 / 159},                                           \
         {1.21 / 159, 1.51 / 159, 1.21 / 159},                                           \
         {0.91 / 159, 0.71 / 159, 0.51 / 159}},                                          \
        {{0.52 / 159, 0.0, 0.52 / 159}, {0.0, 0.0, 0.0}, {0.52 / 159, 0.0, 0.52 / 159}}};
#endif
#endif
...
// template<class REAL, int RESULT_SIZE, int halo, int SMZ_SIZE=halo+1+halo, int REGZ_SIZE=2*halo+1,
// int REGY_SIZE=REG_Y_SIZE_MOD, int REGX_SIZE=2*halo+1, int REG_BASE=halo>
template <class REAL, int RESULT_SIZE, int halo, int REGY_SIZE, int REGZ_SIZE = 2 * halo + 1,
          int REGX_SIZE = 2 * halo + 1, int REG_BASE = halo, int SMZ_SIZE = halo + 1 + halo>
__device__ void __forceinline__ computation(REAL result[RESULT_SIZE], REAL* sm_ptr[SMZ_SIZE],
...
#ifndef BOX
    {
        _Pragma("unroll") for (int l_y = 0; l_y < RESULT_SIZE; l_y++) {
            _Pragma("unroll") for (int hl = 0; hl < halo; hl++) {
                result[l_y] +=
                    west[hl] * sm_ptr[0][sm_width * (l_y + sm_y_base) + sm_x_ind - 1 - hl];
            }
        }
        _Pragma("unroll") for (int l_y = 0; l_y < RESULT_SIZE; l_y++) {
            _Pragma("unroll") for (int hl = 0; hl < halo; hl++) {
                result[l_y] +=
                    east[hl] * sm_ptr[0][sm_width * (l_y + sm_y_base) + sm_x_ind + 1 + hl];
            }
        }
        _Pragma("unroll") for (int l_y = 0; l_y < RESULT_SIZE; l_y++) {
            _Pragma("unroll") for (int hl = 0; hl < halo; hl++) {
                result[l_y] +=
                    north[hl] * sm_ptr[0][sm_width * (l_y + sm_y_base + 1 + hl) + sm_x_ind];
            }
        }
        _Pragma("unroll") for (int l_y = 0; l_y < RESULT_SIZE; l_y++) {
            _Pragma("unroll") for (int hl = 0; hl < halo; hl++) {
                result[l_y] +=
                    south[hl] * sm_ptr[0][sm_width * (l_y + sm_y_base - 1 - hl) + sm_x_ind];
            }
        }
        _Pragma("unroll") for (int l_y = 0; l_y < RESULT_SIZE; l_y++) {
            _Pragma("unroll") for (int hl = 0; hl < halo; hl++) {
    ...
    _Pragma("undroll") for (int l_y = 0; l_y < 2 * halo; l_y++) {
    ...
    _Pragma("unroll") for (int l_y = 0; l_y < RESULT_SIZE; l_y++) {
        {
            int h_y = 2 * halo;
            _Pragma("unroll") for (int h_z = 0; h_z < 2 * halo + 1; h_z++) {
                _Pragma("unroll") for (int h_x = 0; h_x < 2 * halo + 1; h_x++) {
                    reg_ptr[h_z][h_y][h_x] =
                        sm_ptr[(h_z)]
                              [sm_width * (l_y + h_y + sm_y_base - halo) + sm_x_ind - halo + h_x];
                    // smbuffer_buffer_ptr[(h_z+halo)][tile_x_with_halo*(l_y +
                    // h_y-halo+ps_y+index_y) + tid_x+ps_x+h_x];
                }
            }
        }

        _Pragma("unroll") for (int hl_z = -halo; hl_z < halo + 1; hl_z++) {
            _Pragma("unroll") for (int hl_y = -halo; hl_y < halo + 1; hl_y++) {
                _Pragma("unroll") for (int hl_x = -halo; hl_x < halo + 1; hl_x++) {
                    result[l_y] += filter[hl_z + halo][hl_y + halo][hl_x + halo] *
                                   // sm_ptr[hl_z+halo][sm_width*(l_y+halo + hl_y+ sm_y_base-halo) +
                                   // sm_x_ind-halo+hl_x+halo];
...
__global__ void kernel3d_restrict(REAL* input, REAL* output, int height, int width_y, int width_x);
...
__global__ void kernel3d_baseline(REAL* __restrict__ input, REAL* __restrict__ output, int height,
                                  int width_y, int width_x);
#define PERKS_DECLARE_INITIONIZATION_BASELINE(_type, halo, ipt, tilex, blockdim) \
    __global__ void kernel3d_baseline<_type, halo, ipt, tilex, blockdim>(        \
        _type* __restrict__, _type* __restrict__, int, int, int);
...
//     __global__ void
//     kernel3d_baseline_memwarp<_type,halo,ipt,tilex,tiley>(_type*__restrict__,_type*__restrict__,int,int,int);

template <class REAL, int halo, int ipt, int tilex, int blockdim = 256>
__global__ void kernel3d_persistent(REAL* __restrict__ input, REAL* __restrict__ output, int height,
...
#define PERKS_DECLARE_INITIONIZATION_PERSISTENT(_type, halo, ipt, tilex, blockdim) \
    __global__ void kernel3d_persistent<_type, halo, ipt, tilex, blockdim>(        \
        _type* __restrict__, _type* __restrict__, int, int, int, _type*, _type*, int);
...
template <class REAL, int halo, int LOCAL_ITEM_PER_THREAD, int tilex, int reg_folder_z = 0,
          int blocktype, bool UseSMCache = false, int BLOCKDIM = 256,
          int minblocks = getminblocks<REAL, blocktype, LOCAL_ITEM_PER_THREAD>::val>
__global__ void kernel3d_general(REAL* __restrict__ input, REAL* __restrict__ output, int height,
                                 int width_y, int width_x, REAL* l2_cache_i, REAL* l2_cache_o,
                                 int iteration, int max_sm_flder = 0);

#define PERKS_DECLARE_INITIONIZATION_GENERAL(_type, halo, ipt, tilex, regf, minblocks, usesm,   \
                                             blckdim)                                           \
    __global__ void kernel3d_general<_type, halo, ipt, tilex, regf, minblocks, usesm, blckdim>( \
        _type* __restrict__, _type* __restrict__, int, int, int, _type*, _type*, int, int);

#include "../perksconfig.cuh"
#include "./cuda_common.cuh"
template <class REAL, int halo, int LOCAL_ITEM_PER_THREAD, int LOCAL_TILE_X, int registeramount,
          bool UseSMCache, int BLOCKDIM, int shape = star_shape,
          int minblocks = 256 / registeramount>
__global__ void kernel3d_general_wrapper(REAL* __restrict__ input, REAL* __restrict__ output,
                                         int height, int width_y, int width_x, REAL* l2_cache_i,
                                         REAL* l2_cache_o, int iteration,
                                         volatile int* iteration_done, int max_sm_flder = 0);

#define PERKS_DECLARE_INITIONIZATION_GENERAL_WRAPPER(_type, halo, ttile, tilex, ramount, usesm, \
                                                     blockdim, shape)                           \
    __global__ void                                                                             \
    kernel3d_general_wrapper<_type, halo, ttile, tilex, ramount, usesm, blockdim, shape>(       \
        _type* __restrict__, _type* __restrict__, int, int, int, _type*, _type*, int,           \
        volatile int*, int);
