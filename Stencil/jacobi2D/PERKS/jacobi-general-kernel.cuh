...
__device__ __forceinline__ void inner_general(REAL* input, int width_y, int width_x,
                                              const int iy_start, const int iy_end, REAL* __var_4__,
                                              REAL* l2_cache_o, REAL* l2_cache_i, int iteration,
...
#define REG2REG reg2reg
    ...
    const int p_y =
        blockIdx.y * (blocksize_y) + (blockIdx.y <= y_quotient ? blockIdx.y : y_quotient) + 2;
    ...
    //  #pragma unroll
    if (UseRegCache) {
        global2reg<REAL, sizeof_rspace, total_reg_tile_y>(input, r_space, p_y, width_y, p_x + tid,
                                                          width_x, halo);
    }
    ...
    if (UseRegCache || UseSMCache) {
        for (int local_y = tid; local_y < boundary_line_size; local_y += blockDim.x) {
            for (int l_x = 0; l_x < halo; l_x++) {
                ...
                boundary_buffer[e_step + local_y + l_x * boundary_line_size] =
                    input[MIN((p_y + local_y), width_y - 1) * width_x + global_x];
                // west
                global_x = p_x - halo + l_x;
                global_x = MAX(0, global_x);
                boundary_buffer[w_step + local_y + l_x * boundary_line_size] =
                    input[MIN((p_y + local_y), width_y - 1) * width_x + global_x];
            }
        }
        // sdfa
    ...
    for (int iter = 0; iter < iteration; iter++) {
        ...
        // north south
        {
            ...
            if (UseRegCache || UseSMCache) {
                ...
                if (UseSMCache) {
                    ...
                    global2sm<REAL, isBOX, false, true>(
                        input, sm_space, halo, p_y + total_reg_tile_y + total_sm_tile_y, width_y,
                        p_x, width_x, ps_y + total_sm_tile_y, ps_x, tile_x_with_halo, tid);
                } else {
                    _Pragma("unroll") for (int l_y = 0; l_y < halo; l_y++) {
                        ...
                        // need to deal with boundary
                        r_space[total_reg_tile_y + halo + l_y] =
                            (input[(global_y)*width_x + p_x + tid]);
                    }
                }
                if (UseRegCache && UseSMCache) {
                    _Pragma("unroll") for (int l_y = 0; l_y < halo; l_y++) {
                        // north of register
                        r_space[total_reg_tile_y + halo + l_y] =
                            sm_space[(ps_y + l_y) * tile_x_with_halo + tid + ps_x];
                        // south of sm
                    }
                }
            }
...
#ifdef BOX
            /* code */
            SM2REG<REAL, sizeof_rbuffer, halo * 2, isBOX>(sm_rbuffer, r_smbuffer, 0, ps_x, tid,
                                                          tile_x_with_halo);
#endif
        } else {
            SM2REG<REAL, sizeof_rbuffer, halo * 2, isBOX>(sm_rbuffer, r_smbuffer, 0, ps_x, tid,
        ...
        if (UseRegCache) {
            _Pragma("unroll") for (int local_y = 0; local_y < total_reg_tile_y;
                                   local_y += LOCAL_TILE_Y) {
                // deal with ew boundary
                _Pragma("unroll") for (int l_y = tid; l_y < LOCAL_TILE_Y + isBOX;
                                       l_y += blockDim.x) {
                    _Pragma("unroll") for (int l_x = 0; l_x < halo; l_x++) {
                        // east
                        sm_rbuffer[(l_y + ps_y) * tile_x_with_halo + tile_x + ps_x + l_x] =
                            boundary_buffer[e_step + l_y + local_y + l_x * boundary_line_size];
                        // west
                        sm_rbuffer[(l_y + ps_y) * tile_x_with_halo + (-halo) + ps_x + l_x] =
                            boundary_buffer[w_step + l_y + local_y + l_x * boundary_line_size];
                    }
                }
#ifndef BOX
                reg2sm<REAL, sizeof_rspace, LOCAL_TILE_Y>(r_space, sm_rbuffer, ps_y, ps_x, tid,
                ...
                reg2sm<REAL, sizeof_rspace, LOCAL_TILE_Y + 2 * halo>(
                    r_space, sm_rbuffer, ps_y - halo, ps_x, tid, tile_x_with_halo, local_y);
#endif
                __syncthreads();
#ifdef BOX
                sm2regs_nomiddle<REAL, sizeof_rbuffer, LOCAL_TILE_Y, isBOX>(
                    sm_rbuffer, r_smbuffer, 2 * halo, ps_x, tid, tile_x_with_halo, 2 * halo);
// _Pragma("unroll")
...
#ifdef BOX
                computation<REAL, LOCAL_TILE_Y, halo, sizeof_rbuffer, sizeof_rspace>(
                    sum, sm_rbuffer, ps_y, local_x + ps_x, tile_x_with_halo, r_smbuffer, r_space,
                    halo + local_y, stencilParaInput);
// computation<REAL,LOCAL_TILE_Y,halo>(sum,
// sm_rbuffer, ps_y, local_x+ps_x, tile_x_with_halo,
// r_smbuffer, halo,
// stencilParaInput);
#else
                computation<REAL, LOCAL_TILE_Y, halo, sizeof_rspace>(
                    sum, sm_rbuffer, ps_y, local_x + ps_x, tile_x_with_halo, r_space,
                    local_y + halo, stencilParaInput);
#endif
                // __syncthreads();
                // _Pragma("unroll")
...
#ifdef BOX
                // ptrselfcp<REAL,-halo, halo,halo>(sm_rbuffer, ps_y, LOCAL_TILE_Y, tid,
                // tile_x_with_halo);
            ...
            SM2REG<REAL, sizeof_rbuffer, halo * 2, isBOX>(sm_rbuffer, r_smbuffer, 0, ps_x, tid,
                                                          tile_x_with_halo);
#else
            _Pragma("unroll") for (int i = 0; i < 2 * halo; i++) {
        ...
        // computation of share memory space
        {
            ...
            for (int local_y = tid; local_y < total_sm_tile_y + isBOX; local_y += blockDim.x) {
                ...
                for (int l_x = 0; l_x < halo; l_x++) {
                    // east
                    sm_space[(ps_y + local_y) * tile_x_with_halo + tile_x + ps_x + l_x] =
                        boundary_buffer[e_step + local_y + total_reg_tile_y +
                                        l_x * boundary_line_size];
                    // west
                    sm_space[(ps_y + local_y) * tile_x_with_halo + (-halo) + ps_x + l_x] =
                        boundary_buffer[w_step + local_y + total_reg_tile_y +
                                        l_x * boundary_line_size];
                }
            }
            __syncthreads();
            ...
            for (size_t local_y = 0; local_y < total_sm_tile_y; local_y += LOCAL_TILE_Y) {
                ...
                computation<REAL, LOCAL_TILE_Y, halo>(sum, sm_space, ps_y + local_y, local_x + ps_x,
                                                      tile_x_with_halo, r_smbuffer, halo,
                                                      stencilParaInput);
                __syncthreads();
                reg2sm<REAL, LOCAL_TILE_Y, LOCAL_TILE_Y>(sum, sm_space, ps_y + local_y, ps_x, tid,
                                                         tile_x_with_halo, 0);
                __syncthreads();
                REG2REG<REAL, sizeof_rbuffer, sizeof_rbuffer, 2 * halo, isBOX>(
                    r_smbuffer, r_smbuffer, LOCAL_TILE_Y, 0);
        ...
        for (int global_y = p_y_cache_end; global_y < p_y_end; global_y += LOCAL_TILE_Y) {
            global2sm<REAL, halo>(input, sm_rbuffer, LOCAL_TILE_Y, global_y + halo, width_y, p_x,
                                  width_x, ps_y + halo, ps_x, tile_x_with_halo, tid);
            SM2REG<REAL, sizeof_rbuffer, LOCAL_TILE_Y, isBOX>(
                sm_rbuffer, r_smbuffer, 2 * halo, ps_x, tid, tile_x_with_halo, 2 * halo);
            REAL sum[LOCAL_TILE_Y];
            init_reg_array<REAL, LOCAL_TILE_Y>(sum, 0);
            computation<REAL, LOCAL_TILE_Y, halo>(sum, sm_rbuffer, ps_y, local_x + ps_x,
                                                  tile_x_with_halo, r_smbuffer, halo,
                                                  stencilParaInput);
            reg2global<REAL, LOCAL_TILE_Y, LOCAL_TILE_Y>(sum, __var_4__, global_y, p_y_end,
                                                         p_x + local_x, width_x);
            __syncthreads();
            ptrselfcp<REAL, -halo, halo, halo>(sm_rbuffer, ps_y, LOCAL_TILE_Y, tid,
                                               tile_x_with_halo);
            REG2REG<REAL, sizeof_rbuffer, sizeof_rbuffer, 2 * halo, isBOX>(r_smbuffer, r_smbuffer,
                                                                           LOCAL_TILE_Y, 0);
        }
#endif
        gg.sync();
        ...
        if (UseRegCache) {
            if (tid >= blockDim.x - halo) {
                ...
                // east
                //  #pragma unroll
                _Pragma("unroll") for (int l_y = 0; l_y < total_reg_tile_y; l_y++) {
                    boundary_buffer[e_step + l_y + l_x * boundary_line_size] =
                        r_space[l_y];  // sm_space[(ps_y + local_y) * BASIC_TILE_X + TILE_X +
                                       // ps_x-Halo+0];
                }
            } else if (tid < halo) {
                int l_x = tid;
                ...
                //  #pragma unroll
                _Pragma("unroll") for (int l_y = 0; l_y < total_reg_tile_y; l_y++) {
                    boundary_buffer[w_step + l_y + l_x * boundary_line_size] =
                        r_space[l_y];  // sm_space[(ps_y + local_y) * BASIC_TILE_X + TILE_X +
                                       // ps_x-Halo+0];
                }
            }
        ...
        if (UseSMCache) {
            _Pragma("unroll") for (int local_y = tid; local_y < total_sm_tile_y;
                                   local_y += blockDim.x) {
                _Pragma("unroll") for (int l_x = 0; l_x < halo; l_x++) {
                    // east
                    boundary_buffer[e_step + local_y + total_reg_tile_y +
                                    l_x * boundary_line_size] =
                        sm_space[(ps_y + local_y) * tile_x_with_halo + tile_x + ps_x - halo + l_x];
                    // west
                    boundary_buffer[w_step + local_y + total_reg_tile_y +
                                    l_x * boundary_line_size] =
                        sm_space[(ps_y + local_y) * tile_x_with_halo + ps_x + l_x];
                }
            }
        }
        ...
        if (UseRegCache || UseSMCache) {
            _Pragma("unroll") for (int l_y = 0; l_y < halo; l_y++) {
                ...
                if (UseRegCache) {
                    ...
                    __var_4__[(p_y + l_y) * width_x + p_x + tid] =
                        sm_space[(ps_y + l_y) * tile_x_with_halo + tid + ps_x];
                }
            }
        }
        ...
        if (UseSMCache || UseRegCache) {
            _Pragma("unroll") for (int lid = tid; lid < boundary_line_size - isBOX;
                                   lid += blockDim.x) {
                // east
                _Pragma("unroll") for (int l_x = 0; l_x < halo; l_x++) {
                    // east
                    l2_cache_o[(((blockIdx.x * 2 + 1) * halo + l_x) * width_y) + p_y + lid] =
                        boundary_buffer[e_step + lid + l_x * boundary_line_size];
                    // west
                    l2_cache_o[(((blockIdx.x * 2 + 0) * halo + l_x) * width_y) + p_y + lid] =
                        boundary_buffer[w_step + lid + l_x * boundary_line_size];
                }
            }
        }
        gg.sync();
        ...
        if (UseRegCache || UseSMCache) {
            ...
            _Pragma("unroll") for (int local_y = tid; local_y < boundary_line_size - isBOX;
                                   local_y += blockDim.x) {
                _Pragma("unroll") for (int l_x = 0; l_x < halo; l_x++) {
                    ...
                    boundary_buffer[e_step + local_y + l_x * boundary_line_size] =
                        ((blockIdx.x == gridDim.x - 1)
                             ? boundary_buffer[e_step + local_y + (halo - 1) * boundary_line_size]
                             : l2_cache_i[(((blockIdx.x + 1) * 2 + 0) * halo + l_x) * width_y +
                                          cache_y]);
                    // west
                    boundary_buffer[w_step + local_y + l_x * boundary_line_size] =
                        ((blockIdx.x == 0)
                             ? boundary_buffer[w_step + local_y + 0 * boundary_line_size]
                             : l2_cache_i[(((blockIdx.x - 1) * 2 + 1) * halo + l_x) * width_y +
                                          cache_y]);
                }
            }
            for (int local_y = tid; local_y < isBOX; local_y += blockDim.x) {
                for (int l_x = 0; l_x < halo; l_x++) {
                    ...
                    int global_x = p_x + tile_x + l_x;
                    global_x = MIN(width_x - 1, global_x);
                    boundary_buffer[e_step + local_y + boundary_line_size - isBOX +
                                    l_x * boundary_line_size] =
                        input[MIN((p_y + local_y + boundary_line_size - isBOX), width_y - 1) *
                                  width_x +
                              global_x];
                    // west
                    global_x = p_x - halo + l_x;
                    global_x = MAX(0, global_x);
                    boundary_buffer[w_step + local_y + boundary_line_size - isBOX +
                                    l_x * boundary_line_size] =
                        input[MIN((p_y + local_y + boundary_line_size - isBOX), width_y - 1) *
                                  width_x +
                              global_x];
        ...
        if (UseRegCache) {
            _Pragma("unroll") for (int l_y = total_reg_tile_y - 1; l_y >= 0; l_y--) {
    ...
    if (UseRegCache) {
        // register->global
        reg2global<REAL, sizeof_rspace, total_reg_tile_y, false>(r_space, __var_4__, p_y, width_y,
                                                                 p_x + tid, width_x, 0);
    }
    ...
    if (UseSMCache) {
        ...
        sm2global<REAL, false>(sm_space, __var_4__, total_sm_tile_y, p_y + total_reg_tile_y,
                               width_y, p_x, width_x, ps_y, ps_x, tile_x_with_halo, tid);
    }
#undef UseRegCache
#undef SM2REG
#undef REG2REG
