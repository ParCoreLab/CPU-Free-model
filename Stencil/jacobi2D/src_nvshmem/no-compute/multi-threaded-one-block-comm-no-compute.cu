...
__global__ void __launch_bounds__(1024, 1)
    ...
    // const int comp_start_iy = ((blockIdx.x / grid_dim_x) * blockDim.y + threadIdx.y + iy_start +
    // 1) * nx; const int comp_start_ix = ((blockIdx.x % grid_dim_x) * blockDim.x + threadIdx.x +
    // 1);
        ...
        if (blockIdx.x == gridDim.x - 1) {
            ...
            for (int ix = comm_start_ix; ix < end_ix; ix += comm_size_ix)
            {
                const real first_row_val = 0.25 * (a[comm_start_iy + ix + 1] +
                                                                  ...
                                                                  halo_buffer_top[cur_iter_mod * nx
            + ix]); a_new[comm_start_iy + ix] = first_row_val;
            }
            */
            ...
            nx + ix] + a[end_iy - nx + ix]); a_new[(iy_end - 1) * nx + ix] = last_row_val;
            }
            */
