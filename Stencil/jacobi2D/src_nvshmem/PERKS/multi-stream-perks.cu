...
__global__ void __launch_bounds__(1024, 1)
    boundary_sync_kernel(real *a_new, real *a, const int iy_start, const int iy_end, const int nx,
                         const int iter_max, real *halo_buffer_top, real *halo_buffer_bottom,
                         uint64_t *is_done_computing_flags, volatile int *iteration_done,
...
int MultiStreamPERKSNVSHMEM::init(int argc, char *argv[]) {
    ...
    // Add extra 1 because PERKS
    a = (real *)nvshmem_malloc(nx * (chunk_size_high + 2 + 1) * sizeof(real));
    a_new = (real *)nvshmem_malloc(nx * 10 * (chunk_size_high + 2 + 1) * sizeof(real));

    CUDA_RT_CALL(cudaMemset(a, 0, nx * (chunk_size + 2 + 1) * sizeof(real)));
    CUDA_RT_CALL(cudaMemset(a_new, 0, nx * (chunk_size + 2 + 1) * sizeof(real)));
